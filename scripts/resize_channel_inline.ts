...
export async function main() {
    ...
    const configResponse = await yellow.sendMessage(configMessage);
    console.log('Config response', configResponse);

    const { networks, brokerAddress: broker } = configResponse.params as {
        networks: Array<{
            chainId: number;
            custodyAddress: string;
            adjudicatorAddress: string;
    ...
    const resizeResponse = await yellow.sendMessage(resizeMessage);

    console.log(`✅ Resize approved by server, executing on-chain...`, resizeResponse.params);
    console.log(resizeResponse.params.state)
    ...
    const previousState = await nitroliteClient.getChannelData(resizeResponse.params.channelId as Hex);

    const resizeState: FinalState = {
        channelId: resizeResponse.params.channelId as Hex,
        intent: resizeResponse.params.state.intent as StateIntent,
        version: BigInt(resizeResponse.params.state.version) as bigint,
        data: resizeResponse.params.state.stateData as Hex,
        allocations: resizeResponse.params.state.allocations as Allocation[],
        serverSignature: resizeResponse.params.serverSignature as Hex,
    };
